  ...
  while (true) {
    ...
    if (! entry) {
      ...
      if (highestLog > curNum.toInt()) {
        ...
        Serial.print(temp);
        writeFile(SD,temp.c_str(),"rw,rx,ry,rz,ax,ay,az,gx,gy,gz,longDir,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16");
      } else {
        temp = "/log"+curNum+".txt";
        Serial.print(temp);
        writeFile(SD,temp.c_str(),"rw,rx,ry,rz,ax,ay,az,gx,gy,gz,longDir,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16");
...
//      Serial.println();
    ...
    BNO_DATA += ",";
    BNO_DATA += getEventData(&angVelocityData);
